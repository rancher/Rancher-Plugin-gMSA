...
import (
	...
	"github.com/aiyengar2/Rancher-Plugin-gMSA/pkg/provider/watcher"
...
var (
	UserProvidedCertificates = map[string][]string{
		"ca":     {"ca.crt", "tls.crt"},
	...
	CopyCertificates = map[string][]string{
		...
		"client": {"ca.crt", "tls.crt", "tls.key", "tls.pfx"},
	}
)
...
func (m *manager) Start(ctx context.Context) error {
	...
	err = watcher.QuitOnChange(ctx, containerCertFiles...)
	if err != nil {
		return err
	}
	...
	err = utils.CreateAndImportPfx(
		m.containerSSL("client", "tls.crt"),
		m.containerSSL("client", "tls.crt"),
		m.containerSSL("client", "tls.pfx"),
	)
	if err != nil {
		return err
	}
